...
#read all folders 
dir.data <- "ssp_modeling/cost-benefits/out/"
target_cb_file <- "ssp_modeling/cost-benefits/output/cba_results.csv"
...
#cb_data$strategy <- gsub("PFLO:ALL", "Low Emission Pathway", cb_data$strategy)
cb_data$strategy <- gsub("PFLO:NDC", "Unconditional", cb_data$strategy)
cb_data$strategy <- gsub("PFLO:LEP", "Conditional", cb_data$strategy)
cb_data$strategy <- gsub("PFLO:Conditional", "Conditional", cb_data$strategy)

table(cb_data$strategy_code)

#create strategy id 
cb_data$strategy_id <- ifelse(cb_data$strategy_code=="PFLO:NDC", 6003,
                         ifelse(cb_data$strategy_code=="PFLO:LEP", 6004,
                           ifelse(cb_data$strategy_code=="PFLO:CONDITIONAL", 6005,
                         cb_data$strategy_code)))
							  
cb_data$ids <- paste(cb_data$variable,cb_data$strategy_id,sep=":")


table(cb_data$strategy, exclude = NULL)
table(cb_data$strategy_id , exclude = NULL)
table(cb_data$strategy_code, exclude = NULL)
